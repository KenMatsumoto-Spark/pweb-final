"use strict";Object.defineProperty(exports, "__esModule", {value: true}); const courses = [
  {
    url:"ads",
    title:"Análise e Desenvolvimento de Sistemas",
    description: "O tecnólogo em Análise e Desenvolvimento de Sistemas analisa, projeta, documenta, especifica, testa, implanta e mantém sistemas computacionais de informação. Esse profissional trabalha, também, com ferramentas computacionais, equipamentos de informática e metodologia de projetos na produção de sistemas. Raciocínio lógico, emprego de linguagens de programação e de metodologias de construção de projetos, preocupação com a qualidade, usabilidade, robustez, integridade e segurança de programas computacionais são fundamentais à atuação desse profissional.",
    image: "https://cdn.pixabay.com/photo/2015/12/04/14/05/code-1076536_640.jpg",
    coordination: {
      name: "Profº Me. Antonio Cesar de Barros Munari",
      email: "cti@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "diurno",
        link: "http://www.fatecsorocaba.edu.br/pdf/ads-diurno.pdf",
        workload: "2880"
      },{
        period: "noturno",
        link: "http://www.fatecsorocaba.edu.br/pdf/ads-noturno.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ANA CAROLINA CAMARGO FRANCISCO",
      "ANGELINA VITORINO DE SOUZA MELARE",
      "ANTONIO CESAR DE BARROS MUNARI",
      "ANTONIO SERGIO BERNARDO",
      "ANTONIO SERGIO BERNARDO",
      "CARLOS HENRIQUE DIAS",
      "CRISTIANE PALOMAR MERCADO",
      "DENILCE DE ALMEIDA OLIVEIRA VELOSO",
      "DIMAS FERREIRA CARDOSO",
      "EDMEIA SOARES PINTO SCATOLA",
      "FERNANDO CESAR MIRANDA",
      "FRANCISCO CARLOS RIBEIRO",
      "ISABEL CRISTINA CANADO DOS SANTOS",
      "JEFFERSON BLAITT",
      "JOAO CARLOS TEIXEIRA DOS SANTOS",
      "JOSÉ CARLOS MOURA",
      "LEVI RODRIGUES MUNHOZ",
      "LOAMI DA SILVA SOUZA",
      "MARIA ANGELICA CALIXTO DE ANDRADE CARDIERI",
      "MARIA DAS GRAÇAS JUNQUEIRA MACHADO TOMAZELA",
      "MARIA JOSE CARDOZO",
      "OSNI PAULA LEITE",
      "PAULO EDSON ALVES FILHO",
      "RENATO JENSEN",
      "ROSANA HELENA NUNES", 
      "SERGIO MORAES"
    ]
  },
  {
    url:"ea",
    title:"Eletrônica Automotiva",
    description: `O tecnólogo em Eletrônica Automotiva:
    <br>
    <br>
    <i class="bi bi-dot"></i>participa de equipes de desenvolvimento de novos produtos, novas tecnologias e subsistemas na área automotiva;<br>
    <i class="bi bi-dot"></i>supervisiona, coordena e orienta equipes na área automotiva; realiza a padronização, mensuração e controle de qualidade nas áreas automotivas de veículos especiais, carga e passeio;<br>
    <i class="bi bi-dot"></i>especifica instrumentos e equipamentos para teste de veículos; oferece assistência, assessoria e consultoria referentes a instrumentos e equipamentos usados em testes de veículos de especiais, carga e passeio;<br>
    <i class="bi bi-dot"></i>elabora orçamentos referentes à instrumentos e equipamentos usados na industria automotiva;<br>
    <i class="bi bi-dot"></i>gerencia e realiza vistorias, perícias, avaliações, arbitramento, laudo e parecer técnico referentemente à áreas afetadas à veículos especiais, carga e de passeio;<br>
    <i class="bi bi-dot"></i>supervisiona, coordena e orienta tecnicamente equipes de uma linha de inspeção veicular;<br>
    <i class="bi bi-dot"></i>é capaz de selecionar e desenvolver novas tecnologias levando-se em conta características técnicas, humanas, econômicas e gerenciais de sistemas automobilístico;<br>
    <i class="bi bi-dot"></i>opera e faz manutenção de equipamentos em inspeção veicular;<br>
    <i class="bi bi-dot"></i>estuda a viabilização técnico-econômica de uma oficina autorizada ou independente;<br>
    <i class="bi bi-dot"></i>supervisiona, coordena e orienta tecnicamente equipes de uma oficina autorizada ou independente.`,
    image: "https://cdn.pixabay.com/photo/2015/07/20/00/57/vintage-car-852239_1280.jpg",
    coordination: {
      name: "Prof. Me. Fernando César Miranda",
      email: "automotiva@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "vespertino",
        link: "http://www.fatecsorocaba.edu.br/pdf/ea-vespertino.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ALEX SANDRO DE SOUZA JBELLE",
      "ANTONIO CARLOS DE OLIVEIRA",
      "ANTONIO GARCIA NETTO",
      "ARTUR ROBERTO MACHADO DE OLIVEIRA MANDL",
      "CARLA PINEDA LECHUGO",
      "CARLOS ROBERTO NOGUEIRA",
      "ELVIO FRANCO DE CAMARGO ARANHA",
      "EVANDRO DE ALMEIDA LEME",
      "FERNANDO CESAR MIRANDA",
      "FULVIO ISRAEL ARANDA DE ALMEIDA",
      "HELENA SETSUKO DEL MASTRO ESPINDOLA",
      "IRVAL CARDOSO DE FARIA",
      "ISABEL CRISTINA CANADO DOS SANTOS",
      "JOAO LUIZ FLORIO",
      "JOSÉ LUIZ ANTUNES DE ALMEIDA",
      "LEANDRO CORREIA DE LIMA",
      "MARCOS ANTONIO CANHADA",
      "MAURICIO FERNANDO VIEIRA",
      "MÔNICA DE OLIVEIRA PINHEIRO DA SILVA",
      "NIRLEI SANTOS DE LIMA",
      "PAULO DOS SANTOS",
      "SERGIO MORAES",
      "SILVIA PIERRE IRAZUSTA",
      "THIAGO RAGOZO CONTIM",
    ]
  },
  {
    url:"fm",
    title:"Fabricação Mecânica",
    description: `
    O tecnólogo em Fabricação Mecânica atua no segmento de fabricação, envolvendo usinagem, conformação, soldagem, montagem e outros processos mecânicos. Planeja, controla e gerencia os diversos processos, atuando no desenvolvimento e melhoria de produtos, dos processos de fabricação e na gestão de projetos, aliando competências das áreas de gestão, qualidade e controle ambiental. Exerce suas atividades em empresas do ramo metal-mecânico, incluindo indústrias manufatureiras e ferramentarias, podendo ainda atuar em institutos e centros de pesquisa, órgãos governamentais, escritórios de consultoria, dentre outros.
    `,
    image: "https://cdn.pixabay.com/photo/2016/03/04/19/36/gears-1236578_1280.jpg",
    coordination: {
      name: "Profº Me. Amilton Cordeiro de Freitas",
      email: "fmec@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "",
        link: "http://www.fatecsorocaba.edu.br/pdf/fm.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "AMILTON JOAQUIM CORDEIRO DE FREITAS",
      "ANDERSON CARLOS MARIANNO",
      "ANDRE BATISTA DE ALMEIDA",
      "ANTONIO GARCIA NETTO",
      "ARTHUR VIEIRA NETTO JUNIOR",
      "ARTUR ROBERTO MACHADO DE OLIVEIRA MANDL",
      "CARLOS HENRIQUE DIAS",
      "CELIO OLDERIGI DE CONTI",
      "CLAUDIO SERGIO SARTORI",
      "DALMO CAVALCANTI",
      "DAVID ALCOLEA",
      "DÉCIO CARDOSO DA SILVA",
      "EDSON AGUIAR XIMENES",
      "ELIDES BORSARI PINTO FERREIRA",
      "ELVIO FRANCO DE CAMARGO ARANHA",
      "FABIANO GREGOLIN DE CAMPOS BUENO",
      "FABIO HENRIQUE DERMENDJIAN",
      "FLORA CARDOSO DA SILVA",
      "FULVIO ISRAEL ARANDA DE ALMEIDA",
      "GUSTAVO PINTO PETRECHEN",
      "IBERE LUIS MARTINS",
      "IGOR PEREIRA FRANCO",
      "IRVAL CARDOSO DE FARIA",
      "ITALIA APARECIDA ZANZARINI IANO",
      "IVAR BENAZZI JUNIOR",
      "JAIR DONIZETI PINTO DA SILVA",
      "JOSÉ CARLOS MOURA",
      "LAURO CARVALHO DE OLIVEIRA",
      "LEANDRO CORREIA DE LIMA",
      "LUIZ ALBERTO BALSAMO",
      "LUIZ CARLOS ROSA",
      "MARCOS CHOGI IANO",
      "MARCOS DORIGAO MANFRINATO",
      "MARGARETE APARECIDA LEME DE ANDRADE",
      "MARIA JOSE CARDOZO",
      "MÔNICA DE OLIVEIRA PINHEIRO DA SILVA",
      "NELSON RAMPIM FILHO",
      "NORBERTO GONCALVES NETO",
      "ODIR CAMARGO",
      "OLIVIA CRISTINA VITULI CHICOLAMI",
      "ORLANDO MIRANDA PEREZ",
      "OSNI PAULA LEITE",
      "RAFAEL GUSTAVO TURRI",
      "ROGERIO BALDASSIN",
      "ROSANA HELENA NUNES",
      "SONIA MARIA DE PINHO GODOY",
      "SUELLEN SIGNER BARTOLOMEI",
      "WALDEMAR BONVENTI JUNIOR",
      "WANDERLEY DO PRADO",
    ]
  },
  {
    url:"ge",
    title:"Gestão Empresarial - Ensino a Distância",
    description: `
    O Tecnólogo em Gestão Empresarial elabora e implementa planos de negócios, utilizando métodos e técnicas de gestão na formação e organização empresarial especificamente nos processos de comercialização, suprimento, armazenamento, movimentação de materiais e no gerenciamento de recursos financeiros e humanos. A habilidade para lidar com pessoas, capacidade de comunicação, trabalho em equipe, liderança, negociação, busca de informações, tomada de decisão em contextos econômicos, políticos, culturais e sociais distintos, são requisitos importantes a esse profissional.

Eixo Tecnológico: Gestão e Negócios

Mercado de trabalho: Em negócio próprio (consultoria, turismo, comércio, indústria etc.); médias e pequenas empresas; setor público e em entidades particulares, tais como cooperativas e associações.
    `,
    image: "https://cdn.pixabay.com/photo/2016/11/19/21/01/analysis-1841158_1280.jpg",
    coordination: {
      name: "",
      email: "" 
    },
    curriculars: [{
        period: "",
        link: "http://www.fatecsorocaba.edu.br/pdf/Matriz-Curricular-Gestao-Empresarial-EAD.pdf",
        workload: "2880"
      }
    ],
    professors:[

    ],
    extraLinks: [
      {
        title: "Clique aqui para acessar o Ambiente Virtual de Aprendizagem (AVA – Moodle)",
        link: "http://eadfatec.cps.sp.gov.br/"
      },
      {
        title: "Clique aqui para acessar os Calendários",
        link: "/calendario"
      }
    ]
  },
  {
    url:"gq",
    title:"Gestão da Qualidade",
    description: `
    O Tecnólogo em Gestão da Qualidade será um profissional que planeja, implementa e audita sistemas de gestão da qualidade e produtividade. Realiza mapeamento de processos organizacionais segundo indicadores de qualidade e produtividade. Elabora e analisa documentação e relatórios de qualidade, considerando normas de qualidade estabelecidas. Desenvolve avaliação sistemática dos procedimentos, práticas e rotinas internas e externas de uma organização. Mobiliza pessoas para agir com qualidade em todas as atividades corporativas. Gerencia e mantém o fluxo de informação e comunicação na empresa. Dissemina a cultura da qualidade e produtividade. Capacita pessoas em procedimentos e rotinas destinados a minimizar a produção fora de conformidade (utilizando softwares específicos). Elabora ferramentas para minimizar a incidência de falhas. Através da análise de dados (Estatística), Six Sigma, conduz programas de melhorias na Qualidade da empresa. Elabora e gerencia estratégias para obtenção de certificações, garantia e auditoria da Qualidade. Desenvolve programa de avaliação de performance produtiva organizacional considerando aspectos quantitativos e qualitativos. Avalia e emite parecer técnico em sua área de formação.?
    <br>
    <br>
    Onde trabalhar
    <br>
    <br>
    Empresas de pequeno, médio e grande porte (atuando em três áreas macros: Qualidade no produto, Qualidade no fornecedor e Implementação de Normas, Sistemas e Auditorias da Qualidade), assessoria e consultoria. Empresas em geral (indústria, comércio e serviços). Órgãos públicos. Institutos e Centros de Pesquisa. Instituições de Ensino, mediante formação requerida pela legislação vigente.?
    
    `,
    image:"https://cdn.pixabay.com/photo/2018/04/08/10/19/businessman-3300907_1280.jpg",
    coordination: {
      name: "Profº Dr. Délvio Venanzi",
      email: "gestao.qualidade @fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "",
        link: "http://fatecsorocaba.edu.br/pdf/gq.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ANA CAROLINA CAMARGO FRANCISCO",
      "ARTUR FRANCO BUENO",
      "ARTUR ROBERTO MACHADO DE OLIVEIRA MANDL",
      "BENEDITO CESAR FERRAZ",
      "CALIXTO SILVA NETO",
      "CARLA PINEDA LECHUGO",
      "DELVIO VENANZI",
      "DILERMANDO PIVA JUNIOR",
      "EVANDRO DE ALMEIDA LEME",
      "FABIANO GREGOLIN DE CAMPOS BUENO",
      "ISABEL CRISTINA CANADO DOS SANTOS",
      "JOSÉ LÁZARO FERRAZ",
      "MARCOS LOPES",
      "MÔNICA DE OLIVEIRA PINHEIRO DA SILVA",
      "NIRLEI SANTOS DE LIMA",
      "PAULO EDSON ALVES FILHO",
      "TALITA ANNUNCIATO RODRIGUES",
      "WANDERLEY DO PRADO",
    ]
  },
  {
    url:"log",
    title:"Logística",
    description: `
    O tecnólogo em Logística é o profissional especializado em armazenagem, distribuição e transporte. Atuando na área logística de uma empresa, planeja e coordena a movimentação física e de informações sobre as operações multimodais de transporte, para proporcionar fluxo otimizado e de qualidade para peças, matérias-primas e produtos. Ele gerencia redes de distribuição e unidades logísticas, estabelecendo processos de compras, identificando fornecedores, negociando e estabelecendo padrões de recebimento, armazenamento, movimentação e embalagem de materiais, podendo ainda ocupar-se do inventário de estoques, sistemas de abastecimento, programação e monitoramento do fluxo de pedidos.
    `,
    image:"https://cdn.pixabay.com/photo/2021/12/06/10/55/hamburg-6849995_640.jpg",
    coordination: {
      name: "Prof. Me. José Itamar Monteiro",
      email: "logistica@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "vespertino",
        link: "http://www.fatecsorocaba.edu.br/pdf/log-vespertino.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ANGELINA VITORINO DE SOUZA MELARE",
      "ANTONIO CESAR SILVA SACCO",
      "ARTUR ROBERTO MACHADO DE OLIVEIRA MANDL",
      "CARLA PINEDA LECHUGO",
      "CARLOS HENRIQUE DIAS",
      "CRISTIANE PALOMAR MERCADO",
      "DÉCIO DE OLIVEIRA",
      "EDMEIA SOARES PINTO SCATOLA",
      "ELAINE DA CONCEICAO PEREIRA COELHO TAVORA SANTOS",
      "ELIDES BORSARI PINTO FERREIRA",
      "EVANDRO DE ALMEIDA LEME",
      "GUSTAVO PINTO PETRECHEN",
      "ISABEL CRISTINA CANADO DOS SANTOS",
      "JOAO CARLOS TEIXEIRA DOS SANTOS",
      "JORGE LUIZ ANTONIO",
      "JOSÉ CARLOS MOURA",
      "JOSÉ ITAMAR MONTEIRO",
      "JOSÉ LÁZARO FERRAZ",
      "MÁRCIO JOSE PEREIRA DE CAMARGO",
      "MARCOS ANTONIO CANHADA",
      "MARCOS LOPES",
      "NELSON RAMPIM FILHO",
      "NIRLEI SANTOS DE LIMA",
      "NORBERTO GONCALVES NETO",
      "OTAVIO HENRIQUE ROSSI PINTO FERNANDES",
      "PAULO DOS SANTOS",
      "REGINA MARIA LORETO DE OLIVEIRA",
      "SERGIO MORAES",
    ]
  },
  {
    url:"ma",
    title:"Manufatura Avançada",
    description: `
    O que o aluno estuda:
    <br>
    O aluno aprenderá a transformar ambientes de manufatura convencional em ambientes mais tecnológicos. Estruturado por blocos temáticos direcionados a projetos, o conteúdo do curso é dividido em processos de manufatura, eletrônica e automação e áreas multidisciplinares. O aluno terá uma base de artes para desenho técnico, matemática para cálculos de projetos e gestão de carreira. O inglês para linguagem técnica também é uma disciplina recorrente no curso.
    <br>
    <br>

    O que o profissional faz:
    <br>
    Atua como um integrador de tecnologia. Além de projetar e desenvolver projetos de manufatura, o profissional é responsável por agregar novas tecnologias voltadas para a Indústria 4.0. Oferece informações tecnológicas relacionadas às principais técnicas de produção empregadas na indústria mundial, tais como materiais e processos de fabricação, realidade aumentada e virtual, Internet das Coisas, Big Data, entre outros. É responsável por identificar dispositivos e ferramentas disponíveis no mercado. Como empreendedor, pode abrir sua própria consultoria ou empresa. Pode ainda atuar em pesquisas.
    <br>
    <br>

    Onde trabalhar:
    <br>
    Indústria, instituição governamental, escola, hospital, centro de pesquisa tecnológica, empresa de geração, transmissão e distribuição de energia.
    `,
    image:"https://cdn.pixabay.com/photo/2014/09/13/21/46/milling-444493_1280.jpg",
    coordination: {
      name: "Prof. Me. Samuel Mendes Franco",
      email: "manufatura.avancada@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "",
        link: "http://www.fatecsorocaba.edu.br/pdf/ma-diurno.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ANDRE BATISTA DE ALMEIDA",
      "ANTONIO GARCIA NETTO",
      "ARTHUR VIEIRA NETTO JUNIOR",
      "CARLA PINEDA LECHUGO",
      "DAVID ALCOLEA",
      "EDEMIR CELSO MANTOVANI",
      "ELPIDIO GILSON CAVERSAN",
      "ELVIO FRANCO DE CAMARGO ARANHA",
      "FABIANO GREGOLIN DE CAMPOS BUENO",
      "FABIO HENRIQUE DERMENDJIAN",
      "FAUSTO CORREA DE LACERDA",
      "FERNANDO CESAR MIRANDA",
      "FRANCISCO DE ASSIS TOTI",
      "GLAUCO TODESCO",
      "GUSTAVO PINTO PETRECHEN",
      "HELENA SETSUKO DEL MASTRO ESPINDOLA",
      "ISABEL CRISTINA CANADO DOS SANTOS",
      "ITALIA APARECIDA ZANZARINI IANO",
      "JOAO LUIZ FLORIO",
      "JOSÉ LÁZARO FERRAZ",
      "LUCIANA SGARBI ROSSINO",
      "LUIZ CARLOS ROSA",
      "MAIRA DE LOURDES REZENDE",
      "MARCOS ANTONIO CANHADA",
      "MARCOS CHOGI IANO",
      "MARIA DAS GRAÇAS JUNQUEIRA MACHADO TOMAZELA",
      "MARIA JOSE NICOLAI SILVA BARBERO",
      "NIRLEI SANTOS DE LIMA",
      "SAMUEL MENDES FRANCO",
      "SERGIO MORAES",
      "SILVIA PIERRE IRAZUSTA",
      "WALDEMAR BONVENTI JUNIOR",
    ]
  },
  {
    url:"pmet",
    title:"Processos Metalúrgicos",
    description: `
    O tecnólogo em Processos Metalúrgicos utiliza os fenômenos envolvidos em processos como: tratamentos térmicos, fundição, siderurgia, laminação, forjamento de metais, sinterização e tratamentos de superfície .São atividades inerentes aos egressos deste Curso o planejamento, gestão, controle dos processos e comercialização dos produtos metalúrgicos, através da seleção, dimensionamento de equipamentos e métodos de fabricação. Incluem-se as atividades de laboratórios de ensaios mecânicos e de ensaios metalográficos, com o domínio da interrelação entre microestrutura, propriedades e aplicações dos produtos metálicos. Este profissional possui competências de gestão ambiental, de pessoas e processos industriais. Pode atuar em diversas empresas do ramo metal-mecânico e ainda no ensino superior.
    `,
    image:"https://cdn.pixabay.com/photo/2017/11/09/10/18/metallurgy-2932943_1280.jpg",
    coordination: {
      name: "Prof. Dra. Suellen Signer Bartolomei",
      email: "metal@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "",
        link: "http://fatecsorocaba.edu.br/pdf/pm.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ARTUR ROBERTO MACHADO DE OLIVEIRA MANDL",
      "CARLA PINEDA LECHUGO",
      "CESAR AUGUSTO ANTONIO",
      "CLAUDIO SERGIO SARTORI",
      "DÉCIO CARDOSO DA SILVA",
      "EDEMIR CELSO MANTOVANI",
      "EVANDRO DE ALMEIDA LEME",
      "FABIO HENRIQUE DERMENDJIAN",
      "FULVIO ISRAEL ARANDA DE ALMEIDA",
      "IGOR PEREIRA FRANCO",
      "ISABEL CRISTINA CANADO DOS SANTOS",
      "JAIR DONIZETI PINTO DA SILVA",
      "JEFFERSON BLAITT",
      "LUCIANA SGARBI ROSSINO",
      "MARCOS ANTONIO CANHADA",
      "MARCOS CHOGI IANO",
      "MARCOS DORIGAO MANFRINATO",
      "MARGARETE APARECIDA LEME DE ANDRADE",
      "MÔNICA DE OLIVEIRA PINHEIRO DA SILVA",
      "NADIA MARCUZ",
      "NASARENO DAS NEVES",
      "NIRLEI SANTOS DE LIMA",
      "OSNI PAULA LEITE",
      "SILVIA PIERRE IRAZUSTA",
      "SUELLEN SIGNER BARTOLOMEI",
      "WILLIAM FORTUNATO DA SILVA",
    ]
  },
  {
    url:"pol",
    title:"Polímeros",
    description: `
    Esse profissional trabalha na fabricação dos Polímeros, compostos químicos utilizados na fabricação de produtos como o plástico, por exemplo. Avalia o desempenho de equipamentos e processos, interpreta fluxogramas de processos, aplica formulação química de polímeros, tintas e vernizes e desenvolve métodos de análises laboratoriais para caracterização dos materiais poliméricos, além de processos de modelagem. O monitoramento da qualidade e dos processos de reciclagem envolvidos; a identificação e acompanhamento das variáveis relevantes, inclusive as referentes ao meio ambiente, são também funções desse profissional.
    `,
    image:"https://cdn.pixabay.com/photo/2012/10/10/10/29/agar-60571_1280.jpg",
    coordination: {
      name: "Profª Ma. Cécile Chaves Hernandez Garcia",
      email: "polimeros@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "noturno",
        link: "http://fatecsorocaba.edu.br/pdf/pol.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ANTONIO CARLOS DE OLIVEIRA",
      "ARTUR ROBERTO MACHADO DE OLIVEIRA MANDL",
      "CARLOS EDUARDO CORREA",
      "CELIO OLDERIGI DE CONTI",
      "DANIEL KOMATSU",
      "ELPIDIO GILSON CAVERSAN",
      "FABIANO GREGOLIN DE CAMPOS BUENO",
      "GILSON PIQUERAS GARCIA",
      "GISELE DE ARAUJO ROCHA",
      "ISABEL CRISTINA ABUD",
      "ISABEL CRISTINA CANADO DOS SANTOS",
      "JAIR DONIZETI PINTO DA SILVA",
      "JOSÉ CARLOS MOURA",
      "LEANDRO CORREIA DE LIMA",
      "LEVI RODRIGUES MUNHOZ",
      "MAIRA DE LOURDES REZENDE",
      "MARIA JOSE CARDOZO",
      "NELSON RAMPIM FILHO",
      "NIRLEI SANTOS DE LIMA",
      "PAULO DOS SANTOS",
      "RENATO MENDES GERMANO",
      "ROGERIO BALDASSIN",
      "THIAGO RAGOZO CONTIM",
      "WANDERLEY DO PRADO",
      "WILLIAM FORTUNATO DA SILVA",
    ]
  },
  {
    url:"pmec",
    title:"Projetos Mecânicos",
    description: `
    O tecnólogo em Projetos Mecânicos está habilitado a realizar projetos, com detalhamento técnico de sistemas mecânicos que envolvam máquinas, motores, instalações mecânicas e termo-mecânicas. Tem conhecimento de todos os materiais usuais em construção mecânica e suas aplicações práticas. Está capacitado a atuar na área de organização industrial mecânica, tanto para processos como para produtos industriais. Domina a técnica do projeto de dispositivos e ferramentas de produção mecânica. Pode dedicar-se ao ensino, à pesquisa tecnológica, bem como realizar vistoria, avaliação e laudo técnico, em seu campo profissional.
    `,
    image:"https://cdn.pixabay.com/photo/2018/04/16/11/32/technical-drawing-3324368_1280.jpg",
    coordination: {
      name: "Prof. Dr. José Carlos Moura",
      email: "pmec@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "",
        link: "http://www.fatecsorocaba.edu.br/pdf/proj.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ANDRE BATISTA DE ALMEIDA",
      "ANTONIO GARCIA NETTO",
      "ARTHUR VIEIRA NETTO JUNIOR",
      "BENEDITO CESAR FERRAZ",
      "CARLA PINEDA LECHUGO",
      "CLAUDIO SERGIO SARTORI",
      "DAVID ALCOLEA",
      "DÉCIO CARDOSO DA SILVA",
      "EDEMIR CELSO MANTOVANI",
      "ELIDES BORSARI PINTO FERREIRA",
      "ELVIO FRANCO DE CAMARGO ARANHA",
      "EVANDRO DE ALMEIDA LEME",
      "FABIO HENRIQUE DERMENDJIAN",
      "FAUSTO CORREA DE LACERDA",
      "FRANCISCO CARLOS RIBEIRO",
      "FRANCISCO DE ASSIS TOTI",
      "FULVIO ISRAEL ARANDA DE ALMEIDA",
      "GILVAN ANTONIO GARCIA",
      "GUSTAVO PINTO PETRECHEN",
      "HELENA SETSUKO DEL MASTRO ESPINDOLA",
      "IBERE LUIS MARTINS",
      "IGOR PEREIRA FRANCO",
      "IRVAL CARDOSO DE FARIA",
      "ITALIA APARECIDA ZANZARINI IANO",
      "IVAR BENAZZI JUNIOR",
      "JOSE ANTONIO ESQUERDO LOPES",
      "JOSÉ CARLOS MOURA",
      "JOSÉ LÁZARO FERRAZ",
      "LAURO CARVALHO DE OLIVEIRA",
      "LUIZ ALBERTO BALSAMO",
      "MARCOS CHOGI IANO",
      "MARGARETE APARECIDA LEME DE ANDRADE",
      "MAURO TOMAZELA",
      "NIRLEI SANTOS DE LIMA",
      "OLIVIA CRISTINA VITULI CHICOLAMI",
      "ORLANDO MIRANDA PEREZ",
      "OSNI PAULA LEITE",
      "PAULO CESAR JULIANO",
      "PAULO DOS SANTOS",
      "RAFAEL GUSTAVO TURRI",
      "ROGERIO BALDASSIN",
      "ROSANA HELENA NUNES",
      "SAMUEL MENDES FRANCO",
      "SONIA MARIA DE PINHO GODOY",
      "SUELLEN SIGNER BARTOLOMEI",
      "WALDEMAR BONVENTI JUNIOR",
      "WILLIAM FORTUNATO DA SILVA",
    ]
  },
  {
    url:"sb",
    title:"Sistemas Biomédicos",
    description: `
    O tecnólogo em Sistemas Biomédicos é responsável por planejar, gerenciar, implantar e manter equipamentos clínicos e médico-hospitalares. Supervisiona e coordena equipes de manutenção e otimização do uso de equipamentos eletro-médicos. Assessora a aquisição, executa a instalação, capacita usuários de equipamentos e sistemas biomédicos, além de participar de equipes de pesquisa aplicada. Responsável também pela implantação e controle das normas de segurança dos equipamentosnos serviços de saúde, pode atuar em hospitais, policlínicas, laboratórios, fabricantes e distribuidoras de equipamentos hospitalares.
    `,
    image:"https://cdn.pixabay.com/photo/2014/12/10/20/48/laboratory-563423_1280.jpg",
    coordination: {
      name: "Profª Me. Joseli Vergara Marins",
      email: "sbio@fatecsorocaba.edu.br" 
    },
    curriculars: [{
        period: "",
        link: "http://www.fatecsorocaba.edu.br/pdf/sb.pdf",
        workload: "2880"
      }
    ],
    professors:[
      "ALEX SANDRO DE SOUZA JBELLE",
      "ANDRE LUIS PASCHOAL",
      "CLAUDIO SERGIO SARTORI",
      "DAVID ALCOLEA",
      "EDEMIR CELSO MANTOVANI",
      "ELAINE CONCEICAO DE OLIVEIRA",
      "FLORA CARDOSO DA SILVA",
      "FRANCISCO CARLOS RIBEIRO",
      "GILVAN ANTONIO GARCIA",
      "IGOR PEREIRA FRANCO",
      "ITALIA APARECIDA ZANZARINI IANO",
      "JOAO LUIZ FLORIO",
      "JOSÉ LUIZ ANTUNES DE ALMEIDA",
      "JOSELI VERGARA MARINS",
      "MARCO ANTONIO FERRARI",
      "MARCOS CHOGI IANO",
      "NELSON RAMPIM FILHO",
      "PAULO EDSON ALVES FILHO",
      "ROSA MARIA SZAROTA",
      "SERGIO MORAES",
      "SILVIA PIERRE IRAZUSTA",
      "TELMA VINHAS CARDOSO",
      "WALDEMAR BONVENTI JUNIOR",
    ]
  }
]; exports.courses = courses
